...
from shapely.geometry.base import BaseMultipartGeometry as _BaseMultipart

# --- Shapely 2.0 compatibility shim --------------------------------------
# Shapely 2.0 removed direct iteration over multipart geometries
# (MultiPolygon, MultiLineString, GeometryCollection); one must use .geoms.
# This codebase relies pervasively on `for g in multi_geom` and the
# `try: iter(geom) / except TypeError` idiom to recurse into geometry.
# Restore the Shapely 1.x behaviour by delegating iteration to .geoms.
# NOTE: only __iter__ is patched; overriding __len__ breaks Shapely's internal
# vectorization decorators (which call len() on geometry arguments).
if not getattr(_BaseMultipart, "_flatcam_iter_patch", False):
    _BaseMultipart.__iter__ = lambda self: iter(self.geoms)
    _BaseMultipart._flatcam_iter_patch = True
